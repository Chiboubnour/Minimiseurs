...
void minimizer_sequence_cpu(const uint64_t* packed_sequence,
                            uint64_t n_bases,
                            ...
                            size_t& n_smers_out)
{
    ...
    auto get_base = [&](uint64_t idx)->std::pair<uint8_t, bool> {
        if (idx >= n_bases) return {0, false};
        uint64_t word = packed_sequence[idx / 8];
        int j = (int)(idx % 8);
        uint8_t byte = (uint8_t)((word >> (8 * j)) & 0xFFu);
        uint8_t enc = (byte >> 1) & 0x3u; 
        return {enc, true};
    };

    auto t0 = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < smer_bases - 1; i++) {
        auto [enc, valid] = get_base((uint64_t)i);
        if (!valid) break;
        current_smer <<= 2;
    ...
    for (size_t i = 0; i < vmins.size(); i++) {
        vhashes[i] = bfc_hash_64(vmins[i], HASH_MASK);
    }
    ...
    while (idx < vhashes.size()) {
        ...
        if (preload < (size_t)WINDOW_SIZE) {
            buffer[preload++] = vhash; 
        } else {
            buffer[WINDOW_SIZE - 1] = vhash;
        }

        if (lastElement != minz) {
            out_minimizers.push_back(minz);
            lastElement = minz;
        }
    }
...
int main(int argc, char** argv) {
    uint64_t n_bases = 256ull * 1024 * 1024; 
    if (argc >= 2) {
    ...
    std::cout << "=====================================\n";
    std::cout << "  CPU Minimizer (sequence -> hash)\n";
    ...
    minimizer_sequence_cpu(packed.data(), n_bases, out, t_smer, t_hash, t_dedup, n_smers);
    ...
    for (int tt = 0; tt < nTests; ++tt) {
        ...
        minimizer_sequence_cpu(packed.data(), n_bases, out_i, ts, th, td, nsm);
        auto t1r = std::chrono::high_resolution_clock::now();
        double ttsec = std::chrono::duration<double>(t1r - t0r).count();
        t_total_v.push_back(ttsec);
